...
public class Othello extends JFrame implements ActionListener {
	...
	if(theState[c]==player){
	    ...
	    return findBracket(c+dir, dir, player, theState);/* to be filled in the exercise */
	} else {
	    return -1;
	}
    }
    ...
    /*
    public Strategy getBStrategy() {
	return bStrategy;
    }

    public Strategy getWStrategy() {
	return wStrategy;
    }
    */
	...
	public int[] getMove(int player, int[] state) {
	    ...
	    for(int p = 11; p <= 88; p++) {
		int p1 = p % 10;
		if(p1 > 0 && p1 < 9 && legalMove(p, player, state)) {
		    legalMoves[k] = p;
		    k++;
		}
	    };
