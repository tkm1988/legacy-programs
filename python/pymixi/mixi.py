...
class MIXI:
    ...
    def _post_voice(self, body = "", postkey = ""):
        if (body != "") and (postkey != ""):
            params = urllib.urlencode({"body": body,
                                       ...
                                       "post_key": postkey})
            try:
                f = self.opener.open("%s/add_echo.pl" % MIXI_BASE_URL, params)
                data = f.read()
                f.close()
                return BeautifulSoup.BeautifulSoup(data)
            except:
            ...
            try:
                f = self.opener.open("%s/recent_echo.pl" % MIXI_BASE_URL)
                data = f.read()
                f.close()
                return BeautifulSoup.BeautifulSoup(data)
            except:
    ...
    def post_diary(self, title, body):
        ...
        soup = self._post_diary(self.mixiid, unicode(title, 'utf-8').encode('euc-jp'), unicode(body, 'utf-8').encode('euc-jp'), "confirm", postkey)
        if soup == None:
            return False
        return True

    def post_voice(self, body):
        soup = self._post_voice()
        if soup == None:
            return False
        postkey = soup.find("input", {"name": "post_key"})["value"]
        soup = self._post_voice(body, postkey)
        if soup == None:
            return False
        return True
