...
int main(int argc, char *argv[]){
  ...
  string const fpath_err = "/Users/tkm/programming/c/error/broken_evalorg2/";
  ofstream fvq;
  ofstream fl;
  dmat x;
  dmat t;
  ...
  if(argc > 1 && argc <= 7){
    ...
    breakage_rate = strtod(argv[6], NULL);
    ...
    cout << "\t\"/evalorg [Num of RepPt] [leaening time(Per)] [leaening time(SOM)] [dataset's filename(DO NOT INCLUDE EXTENSION)] [none or pp] [breakage rate]\"" << endl;
    exit(-1);
  }
  ...
  fname_learn_err = fpath_err + fname + "_learn_error(k=" + lexical_cast<string>(k) + ", som_lt=" + lexical_cast<string>(som_lt) + ", per_lt=" + lexical_cast<string>(per_lt) + ", br=" + lexical_cast<string>(breakage_rate) + ").dat";
