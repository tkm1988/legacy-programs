...
#include <numbers>
  ...
  if(argc != 4){
    cout << ">./hopfield2 [learning time] [Number of images which you let this program memorize] [breakage rate]" << endl;
    exit(-1);
  }else{
    lt = (int)strtol(argv[1], NULL, 10);
    num =(int)strtol(argv[2], NULL, 10);
    br = strtod(argv[3], NULL);
    ...
    if(br < 0 || br > 100){
      cout << "Error : argv[3] : Out of Range[0 - 100]" << endl;
      exit(-1);
    }
